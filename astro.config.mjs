...
export default defineConfig({
  ...
  integrations: [
      swup({
          ...
          containers: [
              // 旧 Firefly 布局的多个容器（#swup-container / #left-sidebar-dynamic /
              // #right-sidebar-dynamic / #floating-toc-wrapper / #banner-*-container）
              // 已随 MainGridLayout 一起删除，只保留新布局中真实存在的容器。
              // 容器不存在时 swup 会退化为整页刷新（表现为侧栏重置 + 切换变慢）。
              "#swup-container",
          ],
  ...
  vite: {
      ...
      server: {
          ...
          // 允许读取 node_modules 中的 shadcn-svelte/tw-animate-css 基础样式
          fs: {
              allow: [".."],
          },
      },
      resolve: {
          // shadcn-svelte 依赖的 .svelte 包必须交给 Vite 编译，
          // 否则 SSR 构建时会因 Node 无法识别 .svelte 扩展名而失败
          noExternal: ["bits-ui", "@lucide/svelte", "@iconify/svelte", "tailwind-variants"],
          alias: {
              "@rehype-callouts-theme": `rehype-callouts/theme/${siteConfig.rehypeCallouts.theme}`,
              // shadcn-svelte 的 exports 只声明了 style 条件，Tailwind 解析不到，用别名直连
              "shadcn-tailwind-css": fileURLToPath(
                  new URL("./node_modules/shadcn-svelte/dist/tailwind.css", import.meta.url)
              ),
          },
      },
      build: {

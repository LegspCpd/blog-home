  ...
  if (isRestore) {
    for (const rel of DYNAMIC_ROUTES) {
      ...
      if (fs.existsSync(backupFile) && !fs.existsSync(targetFile)) {
        ...
        console.log(`[static-build] Restored src/pages/${rel}`);
      }
    }
  } else {
    for (const rel of DYNAMIC_ROUTES) {
      ...
      if (fs.existsSync(targetFile)) {
        ...
        console.log(`[static-build] Temporarily moved src/pages/${rel} (EdgeOne static build)`);
      }
    }
  }
} catch (err) {
  console.error("[static-build] Error: execution failed:", err.message);
  process.exit(1);
}
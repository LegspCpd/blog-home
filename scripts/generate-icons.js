...
async function loadIconSet(prefix) {
	...
	if (!packageName) {
		console.warn(`Warning: unknown icon set: ${prefix}`);
		return null;
	}
	...
	try {
		...
		console.warn(`Warning: cannot load icon set ${packageName}: ${error.message}`);
		return null;
	}
}
...
async function getIconSvg(iconName) {
	...
	if (!prefix || !name) {
		console.warn(`Warning: invalid icon name: ${iconName}`);
		return null;
	}
	...
	if (!iconData) {
		console.warn(`Warning: icon not found: ${iconName}`);
		return null;
	}
 ...
 */
async function main() {
	console.log("Scanning source files for icon usage...\n");
	...
	console.log(`Discovered ${allIcons.size} distinct icon(s):\n`);
	...
	for (const [prefix, icons] of Object.entries(iconsBySet)) {
		console.log(`   ${prefix}: ${icons.length} icon(s)`);
	}
	console.log("");
	...
	console.log(`Successfully loaded ${successCount} icon(s)`);
	if (failCount > 0) {
		console.log(`Failed to load ${failCount} icon(s)`);
	}
	...
	console.log(`\nWritten: ${OUTPUT_FILE}`);
	console.log(`File size: ${(Buffer.byteLength(output, "utf-8") / 1024).toFixed(2)} KB\n`);
}
